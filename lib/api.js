...
module.exports = function createApi(deps) {
    ...
    // ─── Memory Gateway (emb_002) ───────────────────────────────────────────
    // Read/write .scarlet/ files via API. Path traversal protected.
    const MEMORY_ALLOWED_EXTENSIONS = ['.json', '.jsonl', '.txt', '.md'];
    const MEMORY_MAX_WRITE_SIZE = 64 * 1024; // 64KB max write

    function isMemoryPathSafe(filename) {
        if (!filename || typeof filename !== 'string') return false;
        // No path separators, no dotdot, no absolute paths
        if (filename.includes('/') || filename.includes('\\') || filename.includes('..')) return false;
        if (filename.startsWith('.')) return false;
        const ext = path.extname(filename).toLowerCase();
        return MEMORY_ALLOWED_EXTENSIONS.includes(ext);
    }
    ...
    function handleMemoryRead(req, res) {
        ...
        if (!isMemoryPathSafe(filename)) {
            sendJson(res, 400, { error: 'Invalid filename', allowed_extensions: MEMORY_ALLOWED_EXTENSIONS });
            return;
        }
        const filePath = scarletPath(filename);
        if (!filePath || !fs.existsSync(filePath)) {
            sendJson(res, 404, { error: 'File not found', file: filename });
            return;
        }
        try {
            const stat = fs.statSync(filePath);
            if (stat.size > 256 * 1024) {
                sendJson(res, 413, { error: 'File too large for API read', size: stat.size, max: 256 * 1024 });
                return;
            }
    ...
    function handleMemoryWrite(req, res) {
        if (req.method !== 'PUT') { send405(res, 'PUT'); return; }
        let body = '';
        req.on('data', chunk => {
            body += chunk;
            if (body.length > MEMORY_MAX_WRITE_SIZE) {
                sendJson(res, 413, { error: 'Payload too large', max_bytes: MEMORY_MAX_WRITE_SIZE });
                req.destroy();
            }
        });
        req.on('end', () => {
            try {
                ...
                if (!data.content || typeof data.content !== 'string') {
                    sendJson(res, 400, { error: 'Missing or invalid "content" field' });
                    return;
                }
                const filePath = scarletPath(filename);
                if (!filePath) {
                    sendJson(res, 500, { error: 'Cannot resolve .scarlet/ path' });
                    return;
                }
                // Atomic write via temp file
                const tmpPath = filePath + '.api_tmp';
    ...
    function handleMemoryList(_req, res) {
        const dir = scarletPath('');
        if (!dir) {
            sendJson(res, 500, { error: 'Cannot resolve .scarlet/ directory' });
            return;
        }
        // Remove trailing filename component from scarletPath('')
        const scarletDir = path.dirname(dir);
        try {
            const entries = fs.readdirSync(scarletDir);
            const files = entries
                .filter(e => {
                ...
                .map(name => {
                    try {
                        const stat = fs.statSync(path.join(scarletDir, name));
                        return { name, size: stat.size, modified: stat.mtime.toISOString() };
                    } catch {
                        return { name, size: 0, modified: null };
                    }
                });
            sendJson(res, 200, { directory: '.scarlet/', count: files.length, files });
        } catch (e) {
            sendJson(res, 500, { error: 'List failed', message: e.message });
        }
    }
    ...
    function handleRequest(req, res) {
        ...
        if (pathname === '/status' && req.method === 'GET') {
            ...
            // ?file=X → read; no file param → list
            const parsed2 = url.parse(req.url, true);
            if (parsed2.query.file) {
                handleMemoryRead(req, res);
            } else {
                handleMemoryList(req, res);
            }
        } else if (pathname === '/memory' && req.method === 'PUT') {
            handleMemoryWrite(req, res);
