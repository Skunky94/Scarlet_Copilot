...
module.exports = function createCognition(deps) {
    ...
    // "Reflection deve produrre action candidate" — GPT red team
    // If reflections consistently fail to improve metrics, they're theater.
    const REFLECTION_IMPACT_THRESHOLD = 0.3; // below this → reflection theater
    const REFLECTION_MIN_SAMPLES = 3;        // need at least this many to judge

    function isReflectionTheater() {
        const eff = getReflectionEffectiveness();
        if (eff.total < REFLECTION_MIN_SAMPLES) return false; // insufficient data
        return eff.rate < REFLECTION_IMPACT_THRESHOLD;
    }
    ...
    return {
        ...
        // Reflection effectiveness
        REFLECTION_IMPACT_THRESHOLD,
        REFLECTION_MIN_SAMPLES,
        recordReflection,
        evaluateReflections,
